...
function App() {
  ...
  useEffect(() => {
    ...
    const savedUser = localStorage.getItem('ycmind_user');
    if (savedUser) {
  ...
  const handleLogin = () => {
    ...
    const newUser = {
      id: '1',
      name: 'Huu Tuan',
      email: 'huutuan@example.com'
    };
    setUser(newUser);
    localStorage.setItem('ycmind_user', JSON.stringify(newUser));
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('ycmind_user');
